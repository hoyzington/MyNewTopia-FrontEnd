class Filter {
  ...
  createFilterItems() {
    if (this.items.length == 0) {
      ...
      this.items = this.items.map((item) => {
        return new FilterItem(item.msaAttr, item.vals, item.savedVals, item.hiLoVals)
      })
      this.made = true
      this.saved = true
    }
  ...
  changed() {
    for (const item of this.items) {
      if (FilterItem.different(item.vals, item.savedVals)) { return true }
    }
    return false
  }

  unique() {
    const filters = User.all[0].filters
    if (filters.length > 0) {
      for (const filter of filters) {
        if (this.identicalTo(filter)) { return false }
      }
    }
    return true
  }
  ...
  identicalTo(filter) {
    for (let i = 0; i < 7; i++) {   // Every filter has 7 items
      for (const key in this.items[i].vals) {
        if (!!!filter.items[i].vals[key]) { return false }
      }
      for (const key in filter.items[i].vals) {
        if (!!!this.items[i].vals[key]) { return false }
      }
    }
    return true
  }
  ...
  buildMenuLink() {
    ...
    btn.addEventListener('click', (e) => {
      ...
      this.items.forEach((item, i) => {
        ...
        filter.items[i].vals = Object.assign({}, item.vals)
        filter.items[i].savedVals = Object.assign({}, item.savedVals)
